...
def create(request: TaskAssigneeBase, task_id: uuid.UUID, user_id:uuid.UUID,user=Depends(verify_token)):
    return db_task_assignee.create_task_assignees(request, task_id, user_id, user)


@router.get('/all/{task_id}', response_model=List[TaskAssigneeDisplay])
def all_task_assignees(task_id: uuid.UUID, db=Depends(get_db)):
    return db_task_assignee.get_all_task_assignee(db, task_id)


@router.delete('/delete/{assignee_id}', status_code=status.HTTP_204_NO_CONTENT)
def delete(task_assignee_id: uuid.UUID, db=Depends(get_db)):
    """Delete a task assignee by its ID."""
    if not db_task_assignee.delete(task_assignee_id, db):
